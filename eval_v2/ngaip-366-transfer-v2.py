...
COMMIT_MESSAGE = 'NGAIP-366: Apply v2 RAGAS response accuracy changes'

MODULE = 'from __future__ import annotations\n\nfrom app_retrieval.evaluation.async_ragas_runner import aevaluate_cases, default_metrics\n\nRESPONSE_ACCURACY_METRICS = ["answer_correctness", "answer_relevancy", "faithfulness"]\n\n\nasync def score_response_accuracy(cases, *, llm=None, embeddings=None):\n    return await aevaluate_cases(cases, metrics=default_metrics(RESPONSE_ACCURACY_METRICS), llm=llm, embeddings=embeddings)\n'
TESTS = 'from app_retrieval.evaluation.metrics.response_accuracy_v2 import RESPONSE_ACCURACY_METRICS\n\n\ndef test_response_accuracy_uses_ragas_answer_metrics():\n    assert RESPONSE_ACCURACY_METRICS == ["answer_correctness", "answer_relevancy", "faithfulness"]\n'
...
def main() -> None:
    ...
    path = "app_retrieval/evaluation/metrics/response_accuracy_v2.py"
    test = "tests/app_retrieval/test_response_accuracy_v2.py"
    write(path, MODULE)
    write(test, TESTS)
    run_pytest([test])

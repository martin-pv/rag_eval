#!/usr/bin/env python3
"""NGAIP-364 transfer: configure RAGAS-primary metric wrapper."""
import subprocess
...
def ensure_ticket_branch() -> None:
    print(f"[NGAIP-364] Preparing branch: {BRANCH} from local {BASE_BRANCH}")
...
def commit_transfer_changes() -> None:
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet", "--", *existing], check=False).returncode == 0:
        print(f"[NGAIP-364] No changes to commit."); return
...
TEST_PY = "from __future__ import annotations\n\nfrom app_retrieval.evaluation.metrics.citation_accuracy import METRIC_KEY, OWNER_TICKET, RAGAS_METRIC_SPEC\n\n\ndef test_citation_accuracy_is_ragas_configured():\n    assert METRIC_KEY == 'citation_accuracy'\n    assert OWNER_TICKET == 'NGAIP-364'\n    assert RAGAS_METRIC_SPEC.ragas_symbols\n"
...
def main():
    ...
    ensure(BACKEND / "app_retrieval" / "evaluation" / "metrics" / "citation_accuracy.py", METRIC_PY)
    ensure(BACKEND / "tests" / "app_retrieval" / "test_metric_citation_accuracy.py", TEST_PY)
    commit_transfer_changes()
    print("[NGAIP-364] Done. This ticket now configures RAGAS metrics through the NGAIP-363 adapter.")

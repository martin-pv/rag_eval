#!/usr/bin/env python3
"""NGAIP-366 transfer: configure RAGAS-primary metric wrapper."""
import subprocess
...
BASE_BRANCH = "main"
BACKEND = Path.cwd()
GENERATED_PATHS = ["app_retrieval/evaluation/metrics/__init__.py", "app_retrieval/evaluation/metrics/response_accuracy.py", "tests/app_retrieval/__init__.py", "tests/app_retrieval/test_metric_response_accuracy.py"]
OBSOLETE_PATHS = []
...
def main():
    ...
    ensure(BACKEND / "app_retrieval" / "evaluation" / "metrics" / "response_accuracy.py", METRIC_PY)
    ensure(BACKEND / "tests" / "app_retrieval" / "test_metric_response_accuracy.py", TEST_PY)
    commit_transfer_changes()
    print("[NGAIP-366] Done. This ticket now configures RAGAS metrics through the NGAIP-363 adapter.")

...
# ---------------------------------------------------------------------------
GOLD_PROMOTER_PY = 'from __future__ import annotations\n\nimport json\nfrom pathlib import Path\n\nfrom app_retrieval.evaluation.config.gold_schema import GoldRow\n\n\nPROMOTABLE_STATUSES = {"approved", "edited"}\n\n\ndef promote_candidates_to_gold(candidate_path: Path, gold_path: Path) -> list[GoldRow]:\n    """Promote reviewed RAGAS-generated candidates into the official gold set."""\n    promoted: list[GoldRow] = []\n    for line in candidate_path.read_text(encoding="utf-8").splitlines():\n        if not line.strip():\n            continue\n        row = json.loads(line)\n        if row.get("review_status") not in PROMOTABLE_STATUSES:\n            continue\n        source_metadata = row.get("source_metadata") or []\n        first_source = source_metadata[0] if source_metadata else {}\n        gold_row = GoldRow(\n            question_id=row.get("question_id") or row["candidate_id"],\n            question=row["question"],\n            gold_answer=row["reference_answer"],\n            gold_doc_id=first_source.get("asset_id", "unknown"),\n            gold_chunk_ids=[item["chunk_id"] for item in source_metadata if item.get("chunk_id")] or None,\n            tags=row.get("tags"),\n        )\n        promoted.append(gold_row)\n    gold_path.parent.mkdir(parents=True, exist_ok=True)\n    with gold_path.open("w", encoding="utf-8") as handle:\n        for row in promoted:\n            handle.write(row.model_dump_json(exclude_none=True) + "\\n")\n    return promoted\n'
ensure(ROOT / "app_retrieval" / "evaluation" / "gold_promoter.py", GOLD_PROMOTER_PY)
print("[362-transfer] Created: app_retrieval/evaluation/gold_promoter.py")

# ---------------------------------------------------------------------------
# tests for golden set generation helpers
# ---------------------------------------------------------------------------
TEST_DOC_LOADER_PY = 'import json\n\nfrom langchain_core.documents import Document\n\nfrom app_retrieval.evaluation.langchain_document_loader import load_langchain_documents\n\n\ndef test_load_langchain_documents_preserves_metadata(tmp_path):\n    source = tmp_path / "approved_docs.jsonl"\n    source.write_text(json.dumps({"asset_id": "asset-1", "text": "hello", "chunk_id": "chunk-1"}) + "\\n")\n    docs = load_langchain_documents(source)\n    assert len(docs) == 1\n    assert isinstance(docs[0], Document)\n    assert docs[0].page_content == "hello"\n    assert docs[0].metadata["asset_id"] == "asset-1"\n    assert docs[0].metadata["chunk_id"] == "chunk-1"\n\n\ndef test_load_langchain_documents_rejects_missing_text(tmp_path):\n    source = tmp_path / "bad.jsonl"\n    source.write_text(json.dumps({"asset_id": "asset-1"}) + "\\n")\n    try:\n        load_langchain_documents(source)\n    except ValueError as exc:\n        assert "Missing source document fields" in str(exc)\n    else:\n        raise AssertionError("Expected ValueError")\n'
ensure(ROOT / "tests" / "app_retrieval" / "test_langchain_document_loader.py", TEST_DOC_LOADER_PY)
print("[362-transfer] Created: tests/app_retrieval/test_langchain_document_loader.py")

TEST_KNOWLEDGE_GRAPH_CONTEXT_PY = 'from langchain_core.documents import Document\n\nfrom app_retrieval.evaluation.knowledge_graph_context import attach_knowledge_graph_context, serialize_knowledge_graph_context\n\n\ndef test_serialize_knowledge_graph_context_includes_nodes_and_links():\n    graph = {"nodes": [{"id": "Blade", "group": "part"}], "links": [{"source": "Blade", "target": "Inspection", "predicate": "requires"}]}\n    text = serialize_knowledge_graph_context(graph)\n    assert "Blade" in text\n    assert "requires" in text\n    assert "Inspection" in text\n\n\ndef test_attach_knowledge_graph_context_updates_document_text_and_metadata():\n    docs = [Document(page_content="Source text", metadata={"asset_id": "asset-1"})]\n    enriched = attach_knowledge_graph_context(docs, {"nodes": [{"id": "Entity"}], "links": []})\n    assert "Knowledge graph context" in enriched[0].page_content\n    assert enriched[0].metadata["has_knowledge_graph_context"] is True\n    assert "knowledge_graph_context" in enriched[0].metadata\n'
ensure(ROOT / "tests" / "app_retrieval" / "test_knowledge_graph_context.py", TEST_KNOWLEDGE_GRAPH_CONTEXT_PY)
print("[362-transfer] Created: tests/app_retrieval/test_knowledge_graph_context.py")

TEST_TESTSET_GENERATOR_PY = 'from langchain_core.documents import Document\n\nfrom app_retrieval.evaluation.testset_generator import normalize_ragas_testset\n\n\nclass FakeTestset:\n    def to_list(self):\n        return [{"question": "Q?", "ground_truth": "A.", "contexts": ["ctx"]}]\n\n\ndef test_normalize_ragas_testset_preserves_source_and_graph_metadata():\n    docs = [Document(page_content="ctx", metadata={"asset_id": "asset-1", "chunk_id": "chunk-1", "knowledge_graph_context": "KG"})]\n    rows = normalize_ragas_testset(FakeTestset(), docs)\n    assert rows[0]["question"] == "Q?"\n    assert rows[0]["reference_answer"] == "A."\n    assert rows[0]["source_metadata"][0]["asset_id"] == "asset-1"\n    assert rows[0]["knowledge_graph_context"] == ["KG"]\n    assert rows[0]["review_status"] == "candidate"\n'
ensure(ROOT / "tests" / "app_retrieval" / "test_testset_generator.py", TEST_TESTSET_GENERATOR_PY)
print("[362-transfer] Created: tests/app_retrieval/test_testset_generator.py")

TEST_GOLD_PROMOTER_PY = 'import json\n\nfrom app_retrieval.evaluation.gold_promoter import promote_candidates_to_gold\n\n\ndef test_promote_candidates_to_gold_only_promotes_approved_rows(tmp_path):\n    candidates = tmp_path / "candidates.jsonl"\n    gold = tmp_path / "gold.jsonl"\n    candidates.write_text(\n        json.dumps({"candidate_id": "candidate-0001", "question": "Q?", "reference_answer": "A.", "review_status": "approved", "source_metadata": [{"asset_id": "asset-1", "chunk_id": "chunk-1"}]}) + "\\n" +\n        json.dumps({"candidate_id": "candidate-0002", "question": "Reject?", "reference_answer": "No.", "review_status": "rejected", "source_metadata": [{"asset_id": "asset-2"}]}) + "\\n"\n    )\n    rows = promote_candidates_to_gold(candidates, gold)\n    assert len(rows) == 1\n    assert rows[0].question_id == "candidate-0001"\n    assert rows[0].gold_doc_id == "asset-1"\n    assert rows[0].gold_chunk_ids == ["chunk-1"]\n    assert len(gold.read_text().splitlines()) == 1\n'
ensure(ROOT / "tests" / "app_retrieval" / "test_gold_promoter.py", TEST_GOLD_PROMOTER_PY)
print("[362-transfer] Created: tests/app_retrieval/test_gold_promoter.py")

TEST_GOLDEN_TEST_GENERATOR_PY = 'from types import SimpleNamespace\n\nfrom app_retrieval.evaluation.golden_test_generator import generate_golden_candidates\n\n\ndef test_generate_golden_candidates_wires_loader_factory_generator_and_writer(tmp_path, monkeypatch):\n    calls = []\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.load_eval_config", lambda path: SimpleNamespace(evaluator=SimpleNamespace(enabled=True)))\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.load_langchain_documents", lambda path: ["doc"])\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.attach_knowledge_graph_context", lambda docs, graph_data: ["enriched-doc"])\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.build_ragas_langchain_models", lambda config: ("chat", "embeddings"))\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.build_ragas_azure_completion_llm", lambda config: "completion")\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.generate_candidate_testset", lambda docs, size, generator_llm, critic_llm, embeddings: [{"candidate_id": "candidate-0001"}])\n    monkeypatch.setattr("app_retrieval.evaluation.golden_test_generator.dump_candidate_rows", lambda path, rows: calls.append((path, rows)))\n    output = tmp_path / "candidate_testset.jsonl"\n    rows = generate_golden_candidates(source_docs_path=tmp_path / "approved_docs.jsonl", eval_config_path=tmp_path / "rag_eval.yaml", output_path=output, size=5, graph_data={"nodes": [], "links": []})\n    assert rows == [{"candidate_id": "candidate-0001"}]\n    assert calls == [(output, rows)]\n'
ensure(ROOT / "tests" / "app_retrieval" / "test_golden_test_generator.py", TEST_GOLDEN_TEST_GENERATOR_PY)
print("[362-transfer] Created: tests/app_retrieval/test_golden_test_generator.py")
...
print("  pytest tests/app_retrieval/test_gold_loader.py tests/app_retrieval/test_langchain_document_loader.py tests/app_retrieval/test_knowledge_graph_context.py tests/app_retrieval/test_testset_generator.py tests/app_retrieval/test_gold_promoter.py tests/app_retrieval/test_golden_test_generator.py -v")
print("")

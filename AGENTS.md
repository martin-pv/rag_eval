...
| `toolkit/` | **Poolside** layout + **[`poolside-skills/`](./toolkit/poolside-skills/)** (rag_eval, RAG/RAGAS, **import-claude-skills**) + **[`claude-skills/`](./toolkit/claude-skills/)** (bundled Claude skills) |
...
- Copy skills from **`toolkit/poolside-skills/`** and **`toolkit/claude-skills/`** into **`%USERPROFILE%\.config\poolside\skills\`** (Windows) or **`~/.config/poolside/skills/`** (Unix), or into **`.poolside\skills`** in the project (see **`toolkit/README.md`**). To **pull from another machine’s `.claude` folder** into Poolside or into **`toolkit/claude-skills`**, follow **`toolkit/poolside-skills/import-claude-skills/SKILL.md`**. The **`claude-skills`** tree is also a resolved snapshot; see **`toolkit/claude-skills/README.md`** for the list and refresh instructions.
